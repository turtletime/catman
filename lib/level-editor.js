    ...
    while (true) {
      ...
      const entity = await this.invoke('create-entity',
        Object.assign({}, def, {
          ...
          events: {
            interact: {
            action: `dialog "This ${def.id} was placed here by a higher power."`
            }
          }
        }))
      const onCursorMove = (x, y) => {
        entity.position.x = this.state.scene.camera.toFieldX(x)
        entity.position.y = this.state.scene.camera.toFieldY(y)
        entity.onUpdate()
