...
function executePrivate(path, uiDefMixin) {
  ...
  result.refresh = () => {
    ...
    result.children.forEach(child => child.onChanged())
  }
  result.refresh()
  ...
  result.setText(result.id)

  result.destroy = () => {
    // recursively delete children
    result.children.forEach(child => child.destroy())
    result.parent.children = _.difference(result.parent.children, [result])
    result.parent.sceneTreeNode.removeChild(result.sceneTreeNode)
  }
