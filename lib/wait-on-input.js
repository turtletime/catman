  ...
  async execute(triggers) {
    ...
    await new Promise(resolve => {
      if (onMouseDownActions) {
        onMouseDown = (x, y) => {
          if (onMouseDownActions.find(trigger => trigger.cb ? trigger.cb(x, y) : true)) {
            resolve()
          }
        }
      ...
      if (onKeyDownActions) {
        onKeyDown = (keyCode) => {
          if (onKeyDownActions.find(trigger => this.constants.input[trigger.key] === keyCode && (trigger.cb ? trigger.cb() : true))) {
            resolve()
          }
        }
